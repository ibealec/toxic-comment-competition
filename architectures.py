...
def BidConvPoolGru(maxlen, max_features, embed_size, embedding_matrix):
    inp = Input(shape=(maxlen, ))

    x = Embedding(max_features, embed_size, weights=[embedding_matrix], trainable=False)(inp)
    x = SpatialDropout1D(0.2)(x)
    x = Bidirectional(GRU(512, return_sequences = True, dropout = 0.1, recurrent_dropout=0.1))(x)
...
def CapsuleModel(maxlen, max_features, embed_size, embedding_matrix):
    gru_len = 128
    Routings = 5
    Num_capsule = 10
    Dim_capsule = 16
...
def PureConv(maxlen, max_features, embed_size, emedding_matrix):
    
    inp = Input(shape=(maxlen, ))
    x = Embedding(max_features, embed_size, weights=[embedding_matrix])(inp)
    x = SpatialDropout1D(0.2)(x)
    x = GlobalMaxPooling1D()(x)
    conv = GlobalMaxPool1D()(Conv1D(embed_size, 4, activation="relu")(x))
...
def ConvCapsule(maxlen, max_features, embed_size, emedding_matrix):
    Routings = 5
    Num_capsule = 10
    Dim_capsule = 16
    
    inp = Input(shape=(maxlen, ))
    x = Embedding(max_features, embed_size, weights=[emedding_matrix])(inp)
    x = SpatialDropout1D(0.2)(x)
    # x = GlobalMaxPooling1D()(x)
    conv = Conv1D(embed_size, 4, activation="relu")(x)
    capsule = Capsule(num_capsule=Num_capsule, dim_capsule=Dim_capsule, routings=Routings,
                      share_weights=True)(x)
    # output_capsule = Lambda(lambda x: K.sqrt(K.sum(K.square(x), 2)))(capsule)
    capsule = Flatten()(capsule)
    x = Dropout(0.25)(capsule)
    x = Dense(6, activation="sigmoid")(x)
    model = Model(inputs=inp, outputs=x)
    model.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
    return model     
